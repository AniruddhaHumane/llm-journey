...
class AttnHead:
    ...
    def __init__(self, input_shape, d_head):
        self.d_head = d_head
...
class MultiHeadedAttn:
    def __init__(self, input_shape, d_heads, n_heads, d_model):
        if d_heads % n_heads:
            raise ValueError("d_k must be divisible by n_heads")
        self.heads = [AttnHead(input_shape, d_heads // n_heads) for i in range(n_heads)]
        self.W = np.random.rand(d_heads, d_model)
...
if __name__ == "__main__":
    ...
    mha = MultiHeadedAttn(input_shape, d_k, n_heads, d_model)
    v = mha(X)
    print(v.shape)
