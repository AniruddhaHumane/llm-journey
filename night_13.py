...
if __name__ == "__main__":
    ...
    train_data, val_data = Dataset.from_list(data[:split]), Dataset.from_list(data[split:])
    ...
    trainer = SFTTrainer(model, train_dataset=train_data, eval_dataset=val_data, args=args)
    ...
    with torch.inference_mode():
        ...
        generated_tokens1 = trainer.model.generate(tokens, max_new_tokens=16, temperature=0.3, do_sample=True, pad_token_id=tokenizer.pad_token_id,)
        ...
        generated_tokens1 = trainer.model.generate(tokens, max_new_tokens=16, temperature=0.7, do_sample=True, pad_token_id=tokenizer.pad_token_id,)
        response = tokenizer.decode(generated_tokens1, skip_special_tokens=True)
        print(response)