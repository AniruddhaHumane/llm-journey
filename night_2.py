...
def softmax(arr: np.array, axis=-1):
    return np.exp(arr - np.max(arr, axis=-1).reshape(-1, 1))/np.sum(np.exp(arr - np.max(arr, axis=-1).reshape(-1, 1)), axis=-1).reshape(-1, 1)
...
def plot_mat(matrix: np.array, title=None):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            text_color = "white" if matrix[i, j] > .60 else "black"
            # ax.text(x, y, string, horizontalalignment, verticalalignment)
            ax.text(j, i, str(matrix[i, j].round(3)), 
                    ha="center", 
                    va="center", 
                    color=text_color,
                    fontsize=8)
