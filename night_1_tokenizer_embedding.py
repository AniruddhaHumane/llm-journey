...
print(
    f"gpt2 target json tokens: {[tokenizer.decode([a]) for a in tokens]}, len(tokens): {len(tokens)}"
)
tokens = tokenizer.encode(
    "Blue blouse blossomed with blouse, long sleeves, v-neckline, cotton fabric, floral pattern"
)
print(
    f"gpt2 target sentence tokens: {[tokenizer.decode([a]) for a in tokens]}, len(tokens): {len(tokens)}"
)
print(
    f"gpt2 config: hidden_size: {model.config.hidden_size}, vocab_size: {model.config.vocab_size}"
)
print(f"gpt2 input embedding shape: {model.get_input_embeddings().weight.shape}")
print(
    f"extract specific tokens embedding: {model.get_input_embeddings().weight[tokens].shape}"
)
